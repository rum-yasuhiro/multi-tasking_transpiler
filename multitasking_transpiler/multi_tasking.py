...
from multitasking_transpiler.multi_pm import multi_tasking_pass_manager


def multitasking_transpile(
    multi_circuits: Union[QuantumCircuit, List[QuantumCircuit]],
    backend=None,
    ...
    crosstalk_prop: Optional[Dict[Tuple[int], Dict[Tuple[int], int]]] = None,
    multi_opt=False,
    ...
    multi_programming_circuit = list(
        map(_multitasking_transpile, list(zip(multi_circuits, transpile_args)))
    )
...
def _multitasking_transpile(
    circuit_config_tuple: Tuple[List[QuantumCircuit], Dict]
) -> QuantumCircuit:
    ...
    if multi_opt and isinstance(crosstalk_prop, Dict):
        pass_manager = multi_tasking_pass_manager(pass_manager_config, crosstalk_prop)
...
def _compose_multicircuits(
    circuits: List[QuantumCircuit], transpile_config: Dict
) -> QuantumCircuit:
...
def _remap_circuit_faulty_backend(circuit, num_qubits, backend_prop, faulty_qubits_map):
    ...
    disconnected_qubits = {
        k for k, v in faulty_qubits_map.items() if v is None
    ...
    for virtual, physical in layout.get_virtual_bits().items():
        if faulty_qubits_map[physical] is None:
            raise TranspilerError(
                "The initial_layout parameter refers to faulty"
            ...
            if backend_properties and (
                ...
                for gate in backend_properties.gates:
                    ...
                    if (
                        any(
                            [
                                faulty_qubits_map[qubits] is not None
...
def _parse_output_name(output_name, num_circuits):
    ...
    if output_name is not list:
        if num_circuits != 1 and not isinstance(output_name, None):
            output_name = [str(output_name) + "_" + str(i) for i in range(num_circuits)]
        else:
            output_name = [None] * num_circuits
    return output_name
